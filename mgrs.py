# Credit to aydink on GitHub for pymgrs. MIT licensed.
# https://github.com/aydink/pymgrs
...
def LLtoUTM(lat, lon):
    ...
    M = a * ((1 - eccSquared / 4 - 3 * eccSquared * eccSquared / 64 - 5 * eccSquared * eccSquared * eccSquared / 256) * LatRad - (3 * eccSquared / 8 + 3 * eccSquared * eccSquared / 32 + 45 * eccSquared * eccSquared * eccSquared / \
             1024) * math.sin(2 * LatRad) + (15 * eccSquared * eccSquared / 256 + 45 * eccSquared * eccSquared * eccSquared / 1024) * math.sin(4 * LatRad) - (35 * eccSquared * eccSquared * eccSquared / 3072) * math.sin(6 * LatRad))
    ...
    # return {'northing': math.round(UTMNorthing), 'easting':
    # math.round(UTMEasting), 'zoneNumber': ZoneNumber, 'zoneLetter':
...
def encode(utm, accuracy):
    ...
    return str(utm['zoneNumber']) + utm['zoneLetter'] + str(get100kID(utm['easting'],
                                                                      utm['northing'],
                                                                      utm['zoneNumber'])) + seasting[-5:][0:accuracy] + snorthing[-5:][0:accuracy]
...
def getLetter100kID(column, row, parm):
    ...
    if (colInt == I or (colOrigin < I and colInt > I)
            or ((colInt > I or colOrigin < I) and rollover)):
        colInt += 1

    if (colInt == O or (colOrigin < O and colInt > O)
            or ((colInt > O or colOrigin < O) and rollover)):
        colInt += 1
    ...
    if (((rowInt == I) or ((rowOrigin < I) and (rowInt > I)))
            or (((rowInt > I) or (rowOrigin < I)) and rollover)):
        rowInt += 1

    if (((rowInt == O) or ((rowOrigin < O) and (rowInt > O)))
            or (((rowInt > O) or (rowOrigin < O)) and rollover)):
        rowInt += 1
...
def decode(mgrsString):
    ...
    return {'easting': easting, 'northing': northing, 'zoneLetter': zoneLetter,
            'zoneNumber': zoneNumber, 'accuracy': accuracyBonus}
...
def UTMtoLL(utm):
    ...
    lat = phi1Rad - (N1 * math.tan(phi1Rad) / R1) * (D * D / 2 - (5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * eccPrimeSquared) * D * \
                     D * D * D / 24 + (61 + 90 * T1 + 298 * C1 + 45 * T1 * T1 - 252 * eccPrimeSquared - 3 * C1 * C1) * D * D * D * D * D * D / 720)
    lat = radToDeg(lat)

    lon = (D - (1 + 2 * T1 + C1) * D * D * D / 6 + (5 - 2 * C1 + 28 * T1 - 3 * C1 * C1 + \
           8 * eccPrimeSquared + 24 * T1 * T1) * D * D * D * D * D / 120) / math.cos(phi1Rad)
...
if __name__ == '__main__':
    ...
    print(MGRStoLL("11SNV2866802095"))
